...
function $ToolWindow($component) {
  ...
  $w.$titlebar = $(E("div")).addClass("xp-title-bar").appendTo($w);

  $w.$title = $(E("div")).addClass("xp-title-bar-text").appendTo($w.$titlebar);
  ...
  $w.$content = $(E("div")).addClass("xp-window-body").appendTo($w);
  ...
  $w.applyBounds = () => {
    $w.css({
      left: Math.max(
        0,
        Math.min(innerWidth - $w.width(), $w[0].getBoundingClientRect().left)
      ),
      top: Math.max(
        0,
        Math.min(innerHeight - $w.height(), $w[0].getBoundingClientRect().top)
      ),
    });
  };
