class OnCanvasSelection extends OnCanvasObject {
        ...
        make_or_update_undoable(
          {
            match: (history_node) =>
              (e.shiftKey &&
                history_node.name.match(/^(Smear|Stamp|Move) Selection$/)) ||
  ...
  update_tool_transparent_mode() {
    const sourceImageData = this.source_canvas.ctx.getImageData(
      0,
      0,
      this.width,
      this.height
    );
    const cutoutImageData = this.canvas.ctx.createImageData(
      this.width,
      this.height
    );
  ...
  scale(factor) {
    const new_source_canvas = make_canvas(
      this.width * factor,
      this.height * factor
    );
    new_source_canvas.ctx.drawImage(
