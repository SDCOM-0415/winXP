  ...
  $b.update = (_color) => {
    ...
    requestAnimationFrame(() => {
      ...
      if (color) {
        ...
        swatch_canvas.ctx.fillRect(
          0,
          0,
          swatch_canvas.width,
          swatch_canvas.height
        );
      }
    });
  };
...
function $ColorBox(vertical) {
  ...
  const $foreground_color = $Swatch(colors.foreground).addClass(
    "color-selection foreground-color"
  );
  const $background_color = $Swatch(colors.background).addClass(
  ...
  function color_to_hex(col) {
    ...
    const rgb = rgb_match
      ? rgb_match.slice(1)
      : get_rgba_from_color(col).slice(0, 3);
    function hex(x) {
  ...
  const build_palette = () => {
    ...
    if (vertical) {
      const height_per_button =
        ...
        parseFloat(
          getComputedStyle($some_button[0]).getPropertyValue("margin-top")
        ) +
        parseFloat(
          getComputedStyle($some_button[0]).getPropertyValue("margin-bottom")
        );
      ...
      const width_per_button =
        ...
        parseFloat(
          getComputedStyle($some_button[0]).getPropertyValue("margin-left")
        ) +
        parseFloat(
          getComputedStyle($some_button[0]).getPropertyValue("margin-right")
        );
