...
function show_custom_zoom_window() {
  ...
  $fieldset.append(
    "<label><input type='radio' name='custom-zoom-radio' value='really-custom'/><input type='number' min='10' max='1000' name='really-custom-zoom-input' value=''/>%</label>"
  );
  let is_custom = true;
  $fieldset
  ...
  $w.$Button("OK", () => {
    ...
    if (option_val === "really-custom") {
      ...
      if (isNaN(mag)) {
        const $msgw = new $FormToolWindow("Invalid Value").addClass(
          "dialogue-window"
        );
...
function load_image_from_URI(uri, callback) {
  ...
  const try_next_uri = () => {
    ...
    const handle_fetch_fail = () => {
      ...
      if (index >= uris_to_try.length) {
        ...
        callback &&
          callback(
            new Error(
              `failed to download image from any of three URIs (${JSON.stringify(
                uris_to_try
              )}).`
            )
          );
    ...
    if (is_download) {
      console.log(
        `Try loading image from URI (${index + 1}/${
          uris_to_try.length
        }): "${uri_to_try}"`
      );
    }
      ...
      .then((blob) => {
        ...
        const handle_decode_fail = () => {
          ...
          fr.onload = (e) => {
            ...
            error.code = e.target.result.match(/^\s*<!doctype\s+html/i)
              ? "html-not-image"
              : "decode-fail";
            callback(error);
          };
...
function open_from_FileList(files, user_input_method_verb_past_tense) {
  ...
  if (files.length > 1) {
    show_error_message(
      `None of the files ${user_input_method_verb_past_tense} appear to be images.`
    );
  } else {
    show_error_message(
      `File ${user_input_method_verb_past_tense} does not appear to be an image.`
    );
  }
}
...
function file_save_as() {
  ...
  save_canvas_as(
    canvas,
    `${file_name.replace(
      /\.(bmp|dib|a?png|gif|jpe?g|jpe|jfif|tiff?|webp|raw)$/,
      ""
    )}.png`,
    (saved_file_path, saved_file_name) => {
      saved = true;
...
function show_about_paint() {
  ...
  const $fieldset = $(E("div")).appendTo($w.$main);
...
function go_to_history_node(target_history_node, canceling) {
  ...
  const current_image_data = ctx.getImageData(
    0,
    0,
    canvas.width,
    canvas.height
  ...
  if (
    ...
    !image_data_match(current_history_node.image_data, current_image_data, 5)
  ) {
    window.console &&
      console.log(
        "Canvas image data changed outside of undoable",
        current_history_node,
        "current_history_node.image_data:",
        current_history_node.image_data,
        "document's current image data:",
        current_image_data
      );
    undoable(
      { name: "Unknown [GTHN]", use_loose_canvas_changes: true },
      () => {}
    );
  }
...
function undoable({ name, icon, use_loose_canvas_changes, soft }, callback) {
  if (!use_loose_canvas_changes) {
    ...
    if (
      ...
      undoable(
        { name: "Unknown [undoable]", use_loose_canvas_changes: true },
        () => {}
      );
    }
  }
  ...
  if (document.queryCommandEnabled(commandId)) {
    ...
    if (!navigator.userAgent.includes("Firefox") || commandId === "paste") {
      return show_error_message(
        `That ${commandId} probably didn't work. ${browserRecommendationForClipboardAccess}`
      );
    }
  } else {
    return show_error_message(
      `Cannot perform ${commandId}. ${browserRecommendationForClipboardAccess}`
    );
  }
}
  ...
  if (text.length > 0) {
    if (!navigator.clipboard || !navigator.clipboard.writeText) {
      if (execCommandFallback) {
        ...
        throw new Error(
          `The Async Clipboard API is not supported by this browser. ${browserRecommendationForClipboardAccess}`
        );
      }
    }
...
function make_monochrome_pattern(lightness) {
  ...
  const pattern_image_data = ctx.createImageData(
...
function image_attributes() {
  ...
  const $units = $(E("fieldset"))
    ...
    .attr(
      "style",
      "background-color: #ece9d8; width: 200px; display: flex; justify-content: space-between;"
    )
    .appendTo($main)
    .append("<legend>Units</legend>");

  $units.append(
    '<label><input type="radio" name="units" value="in">Inches</label>'
  );
  $units.append(
    '<label><input type="radio" name="units" value="cm">Cm</label>'
  );
  $units.append(
    '<label><input type="radio" name="units" value="px">Pixels</label>'
  );
  ...
  $colors.append(
    '<label><input type="radio" name="colors" value="monochrome">Black and White</label>'
  );
  $colors.append(
    '<label><input type="radio" name="colors" value="polychrome">Color</label>'
  );
  $colors
  ...
  $transparency.append(
    '<label><input type="radio" name="transparency" value="transparent">Transparent</label>'
  );
  $transparency.append(
    '<label><input type="radio" name="transparency" value="opaque">Opaque</label>'
  );
  $transparency
...
function image_stretch_and_skew() {
  ...
  const $RowInput = (
    ...
    $(E("td"))
      .appendTo($tr)
      .append($(E("label")).text(label_text).attr("for", input_id));
