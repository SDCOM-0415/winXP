  ...
  window.tools = [
    {
      ...
      pointerup() {
        ...
        undoable(
          {
            name: "Free-Form Select",
            icon: get_icon_for_tool(get_tool_by_name("任意形状选择")),
            soft: true,
          },
          () => {
    ...
    },
    {
      ...
      drawPreviewAboveGrid(
        ...
        const id_src = canvas.ctx.getImageData(
          rect_x,
          rect_y,
          rect_w + 1,
          rect_h + 1
  ...
  tools.forEach((tool) => {
    if (tool.selectBox) {
      ...
      tool.paint = () => {
        ...
        rect_width =
          ~~Math.min(canvas.width, Math.max(drag_start_x, pointer.x) + 1) -
          rect_x;
        rect_height =
          ~~Math.min(canvas.height, Math.max(drag_start_y, pointer.y + 1)) -
          rect_y;
      };
      tool.pointerup = () => {
        $canvas_handles.show();
