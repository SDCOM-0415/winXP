...
const get_brush_canvas = memoize_synchronous_function(
  ...
  20
); // 12 brush tool options + current brush + current pencil + current eraser + current shape stroke + a few
...
const get_circumference_points_for_brush = memoize_synchronous_function(
  (brush_shape, brush_size) => {
    ...
    const at = (x, y) =>
      image_data.data[(y * image_data.width + x) * 4 + 3] > 0;
...
function draw_grid(ctx, scale) {
  ...
  if (
    !grid_pattern ||
    grid_pattern.width !== pattern_size ||
    grid_pattern.height !== pattern_size
  ) {
...
(() => {
  ...
  function draw_dashes(ctx, x, y, go_x, go_y, scale, translate_x, translate_y) {
    if (!vertical_pattern) {
      ...
      horizontal_pattern = ctx.createPattern(
        horizontal_pattern_canvas,
        "repeat"
      );
...
(() => {
  ...
  function createShaderProgram() {
    ...
    if (!gl.getShaderParameter(vertexShader, gl.COMPILE_STATUS)) {
      window.console &&
        console.log(
          `Vertex shader failed to compile. Log: ${gl.getShaderInfoLog(
            vertexShader
          )}`
        );
    }
    ...
    if (!gl.getShaderParameter(fragmentShader, gl.COMPILE_STATUS)) {
      window.console &&
        console.log(
          `Fragment shader failed to compile. Log: ${gl.getShaderInfoLog(
            fragmentShader
          )}`
        );
    }
  ...
  function draw_polygon_or_line_strip(ctx, points, stroke, fill, close_path) {
    ...
    for (let i = 0; i < numPoints; i++) {
      coords[i * 2 + 0] =
        ((points[i].x - x_min) / op_canvas_webgl.width) * 2 - 1;
      coords[i * 2 + 1] =
    ...
    if (stroke) {
      if (stroke_size > 1) {
        ...
        gl.clear(gl.COLOR_BUFFER_BIT);
        gl.drawArrays(
          close_path ? gl.LINE_LOOP : gl.LINE_STRIP,
          0,
          numVertices
        );
  ...
  window.copy_contents_within_polygon = (
    canvas,
    points,
    x_min,
    y_min,
