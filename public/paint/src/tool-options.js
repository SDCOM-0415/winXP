...
const $Choose = (
  ...
  choose,
  is_chosen,
  gray_background_for_unselected
) => {
  ...
  $chooser.on("update", () => {
    ...
    for (let i = 0; i < things.length; i++) {
      ((thing) => {
        const $option_container = $(E("div"))
          .addClass("chooser-option")
          .appendTo($chooser);
        ...
        const update = () => {
          const selected_color = getComputedStyle($chooser[0]).getPropertyValue(
            "--Hilight"
          );
          const unselected_color = gray_background_for_unselected
            ? "rgb(192, 192, 192)"
            : "";
          $option_container.css({
            backgroundColor: is_chosen(thing)
              ? selected_color
              : unselected_color,
          });
...
const $ChooseShapeStyle = () => {
  const $chooser = $Choose(
    ...
    ({ stroke, fill }, is_chosen, reuse_canvas) => {
      ...
      ssctx.fillStyle = is_chosen
        ? style.getPropertyValue("--HilightText")
        : style.getPropertyValue("--WindowText");
...
const $choose_brush = $Choose(
  (() => {
    ...
    brush_shapes.forEach((brush_shape) => {
      const sizes =
        brush_shape === "circle" ? circular_brush_sizes : brush_sizes;
...
const $choose_eraser_size = $Choose(
  ...
  (size, is_chosen, reuse_canvas) => {
    ...
    cecanvas.ctx.fillStyle = is_chosen
      ? style.getPropertyValue("--HilightText")
      : style.getPropertyValue("--WindowText");
...
const $choose_stroke_size = $Choose(
  ...
  (size, is_chosen, reuse_canvas) => {
    ...
    cscanvas.ctx.fillStyle = is_chosen
      ? style.getPropertyValue("--HilightText")
      : style.getPropertyValue("--WindowText");
...
const $choose_magnification = $Choose(
  ...
  (scale, is_chosen, reuse_canvas) => {
    ...
    const chooser_canvas = ChooserCanvas(
      ...
      39,
      secret ? 2 : 13, // width, height of destination canvas
      i * 23,
      0,
      23,
      9, // x, y, width, height from source image
      8,
      2,
      23,
      9, // x, y, width, height on destination
      reuse_canvas
    );
    if (secret) {
...
const $choose_airbrush_size = $Choose(
  ...
  (size, is_chosen, reuse_canvas) => {
    ...
    return ChooserCanvas(
      ...
      is_chosen, // invert if chosen
      w,
      h, // width, height of created destination canvas
      source_x,
      0,
      w,
      h, // x, y, width, height from source image
      0,
      0,
      w,
      h, // x, y, width, height on created destination canvas
      reuse_canvas
    );
  },
  (size) => {
...
const $choose_transparent_mode = $Choose(
  ...
  (option, _is_chosen, reuse_canvas) => {
    ...
    return ChooserCanvas(
      ...
      b + sw + b,
      b + sh + b, // width, height of created destination canvas
      0,
      option ? 22 : 0,
      sw,
      sh, // x, y, width, height from source image
      b,
      b,
      sw,
      sh, // x, y, width, height on created destination canvas
      reuse_canvas
    );
  },
  (option) => {
