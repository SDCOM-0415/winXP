(function (exports) {
  ...
  function $MenuBar(menus) {
    ...
    const _html = (menus_key) =>
      menus_key.replace(
        /&(.)/,
        (m) => `<span class='menu-hotkey'>${m[1]}</span>`
      );
    const _hotkey = (menus_key) =>
      menus_key[menus_key.indexOf("&") + 1].toUpperCase();
    ...
    function $MenuPopup(menu_items) {
      ...
      const $menu_popup_table = $(E("table"))
        .addClass("menu-popup-table")
        .appendTo($menu_popup);

      $.map(menu_items, (item) => {
        const $row = $(E("tr"))
          .addClass("menu-row")
          .appendTo($menu_popup_table);
        if (item === MENU_DIVIDER) {
          ...
          if (item.submenu) {
            ...
            const open_submenu = () => {
              ...
              let submenu_popup_rect =
                $submenu_popup[0].getBoundingClientRect();
    ...
    const make_menu = (menus_key, menu_items) => {
      const $menu_container = $(E("div"))
        .addClass("menu-container")
        .appendTo($menus);
      const $menu_button = $(E("div"))
      ...
      const update_position_from_containing_bounds = () => {
        ...
        if (uncorrected_rect.right > innerWidth) {
          $menu_popup.css(
            "left",
            innerWidth - uncorrected_rect.width - uncorrected_rect.left
          );
        }
      };
      ...
      $menu_container.on("keydown", (e) => {
        ...
        switch (e.keyCode) {
          ...
          case 39: // Right
            if (
              $focused_item.find(".menu-item-submenu-area:not(:empty)").length
            ) {
