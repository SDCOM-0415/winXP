...
| 批次 | 内容 | 状态 |
|---|---|---|
| P0 | 图标补充 / 光标 / 壁纸本地化 | ✅ 已完成 |
| P1 | 窗口装饰位图、系统符号、托盘真实化、24 小时制时钟 | ✅ 已完成 |
| P2 | 可写 VFS + localStorage 持久化 | ✅ 已完成 |
| P3 | Luna ↔ 经典双主题、屏保 | ✅ 已完成（双主题含"显示 属性"入口） |
| — | 3D 弹球 | ⏸ 已记为未来工作，见 `.workbuddy/memory/MEMORY.md` |
| P3+ | 命令行、纸牌、WMP 等具体应用 | ⬜ 未开始 |
| P4 | 录音机、帮助中心、添加删除程序、Outlook、MSN 等 | ⬜ 未开始 |

已落实的用户决策：
- 开机页面**保留现状**（未移植对方的 `res/anim/boot.gif`）
- 壁纸**使用当前这张**并下载进代码（未使用对方的 Bliss.jpg）
- 字体**保留当前设置**（未引入对方字体；`font.css` 的 `Noto Sans TC` 为繁体字库，待用户决定是否改）
- **不需要 appstore**（同时移除了 `ui/tray/appstore.png`）
- 基础设施改造已完整实施（§4.1–§4.5 全部完成）
